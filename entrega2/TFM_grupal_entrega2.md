...
La lengua Inga, variante de la familia quechua hablada por cerca de 18.000 personas en el departamento del Putumayo (Colombia), no cuenta con herramientas digitales de traducción automática. Este trabajo aborda esa brecha mediante el diseño y la implementación de un sistema comparativo bidireccional Inga-español construido sobre dos aproximaciones complementarias. La primera adapta el modelo de traducción multilingüe NLLB-200-distilled-600M al par Inga-español mediante ajuste fino eficiente en parámetros con adaptadores de bajo rango, aprovechando las lenguas quechuas emparentadas que el modelo ya cubre como base de transferencia. La segunda construye un pipeline de traducción sobre el modelo de lenguaje de frontera Claude Sonnet 4.6 potenciado con recuperación aumentada de información, sustentado en una base de conocimiento estructurada en cuatro índices vectoriales: léxico, gramatical y dos índices de ejemplos paralelos para soportar ambas direcciones de traducción. Esta memoria intermedia documenta el cierre de las primeras seis fases del proyecto y los resultados experimentales preliminares. El corpus paralelo alcanzó 5.589 pares verificados a partir de la alineación canónica entre el Nuevo Testamento en Inga y la Reina-Valera 1909 en español, distribuidos en particiones de entrenamiento, validación y prueba. La base de conocimiento operativa contiene 4.900 entradas léxicas, 605 bloques gramaticales y 8.942 ejemplos paralelos indexados con embeddings de Gemini sobre LanceDB. La evaluación comparativa preliminar reportó las cinco configuraciones experimentales en ambas direcciones de traducción con métricas BLEU, chrF++ y BERTScore sobre el conjunto de validación. La contribución original del trabajo reside en la comparación sistemática de adaptación NMT y recuperación aumentada sobre un mismo par lingüístico de recursos extremadamente bajos, en la naturaleza bidireccional del sistema implementado y en la liberación abierta de todos los recursos derivados bajo licencias compatibles con la redistribución para la comunidad Inga.
...
The Inga language, a Quechua variant spoken by about 18,000 people in the department of Putumayo (Colombia), has no machine translation tools available. This work addresses that gap through the design and implementation of a bidirectional comparative Inga-Spanish translation system built on two complementary approaches. The first adapts the NLLB-200-distilled-600M multilingual translation model to the Inga-Spanish pair through parameter-efficient fine-tuning with low-rank adapters, leveraging related Quechua languages already covered by the model as a transfer base. The second builds a translation pipeline on top of the Claude Sonnet 4.6 frontier language model enhanced with retrieval-augmented generation, supported by a knowledge base structured in four vector indices: lexical, grammatical, and two parallel-example indices to support both translation directions. This intermediate report documents the completion of the first six project phases and the preliminary experimental results. The parallel corpus reached 5,589 verified pairs through canonical alignment between the Inga New Testament and the Spanish Reina-Valera 1909, distributed into training, validation, and test partitions. The operational knowledge base contains 4,900 lexical entries, 605 grammatical chunks, and 8,942 indexed parallel examples using Gemini embeddings on LanceDB. The preliminary comparative evaluation reported the five experimental configurations in both translation directions with BLEU, chrF++, and BERTScore metrics on the validation set. The original contribution of this work lies in the systematic comparison of NMT adaptation and retrieval-augmented generation on the same extremely low-resource language pair, in the bidirectional nature of the implemented system, and in the open release of all derived resources under licenses compatible with redistribution to the Inga community.
