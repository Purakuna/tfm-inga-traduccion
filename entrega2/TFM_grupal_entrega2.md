...
El corpus paralelo desarrollado alcanza 5.641 pares verificados, generados mediante alineación canónica entre el Nuevo Testamento en Inga (Wycliffe) y la Reina-Valera 1909 en español, complementados con material jurídico de la Constitución Política de Colombia traducida al Inga por Tandioy Jansasoy y el CCELA de Uniandes. Los pares se distribuyen en particiones de entrenamiento, validación y prueba. La base de conocimiento operativa cubre 4.900 entradas léxicas, 605 bloques gramaticales y 9.024 ejemplos paralelos indexados con embeddings de Gemini sobre LanceDB. La evaluación comparativa de las cinco configuraciones experimentales en ambas direcciones identifica al pipeline LLM con recuperación aumentada como la aproximación con mejor desempeño, alcanzando 17,89 puntos BLEU en la dirección Inga-español y 10,95 en español-Inga sobre el conjunto de validación.
...
The parallel corpus developed in this work reaches 5,641 verified pairs, generated through canonical alignment between the Inga New Testament (Wycliffe) and the Spanish Reina-Valera 1909, complemented with legal material from the Colombian Constitution translated into Inga by Tandioy Jansasoy and the CCELA at Universidad de los Andes. The pairs are distributed into training, validation, and test partitions. The operational knowledge base contains 4,900 lexical entries, 605 grammatical chunks, and 9,024 parallel examples indexed with Gemini embeddings on LanceDB. The comparative evaluation of the five experimental configurations across both directions identifies the LLM pipeline with retrieval-augmented generation as the best-performing approach, reaching 17.89 BLEU points in the Inga-Spanish direction and 10.95 in Spanish-Inga on the validation set.
...
Un aspecto transversal a estos trabajos es la discusión sobre las implicaciones éticas y comunitarias del uso de tecnologías de traducción automática aplicadas a lenguas indígenas. Mager et al. (2023) sistematizaron en el marco de ACL las consideraciones éticas específicas de esta línea de investigación, enfatizando la importancia de la participación comunitaria, el consentimiento informado y el retorno de los beneficios a las comunidades hablantes. El este TFM asume estos principios como parte integral de su diseño, tal como se detalla en el marco normativo y en la sección de organización del trabajo.
...
En conjunto, estos trabajos demuestran que el par tecnológico central del este TFM , ajuste fino eficiente sobre NLLB-200 más estrategias complementarias de recuperación aumentada, es una aproximación viable y alineada con el estado del arte para lenguas de la familia quechua.
...
Un estudio comparativo especialmente relevante para las decisiones metodológicas de este trabajo es el de Su et al. (2024), que evaluó empíricamente ocho métodos PEFT sobre quince arquitecturas distintas en el escenario específico de traducción automática de bajos recursos (*low-resource language translation*, LRL-NMT). El estudio recomienda un conjunto de combinaciones modelo-método según el volumen de datos paralelos disponibles y las capacidades computacionales, y sirve como guía para el diseño experimental del este proyecto.
...
Segundo, el paradigma de modelos de lenguaje de frontera con recuperación aumentada ha emergido como una alternativa complementaria con propiedades favorables específicas para lenguas de bajos recursos. La "eficiencia de recursos" documentada para Claude por Enis y Hopkins (2024), junto con la evidencia sobre *in-context learning few-shot* de Cahyawijaya, Lovenia y Fung (2024), sustentan la segunda línea experimental de este trabajo.

Tercero, existe un vacío específico en la literatura que el este TFM contribuye a cerrar: no se ha identificado ningún trabajo previo que compare sistemáticamente ambas aproximaciones , ajuste fino eficiente de NMT multilingüe versus modelo de lenguaje de frontera con recuperación aumentada, sobre el mismo par lingüístico de bajos recursos, y tampoco trabajos que aborden el Inga con cobertura dialectal explícita de Alto Putumayo y Medio Putumayo. Ambas contribuciones constituyen el aporte original del proyecto.
...
Este capítulo documenta la implementación de las Fases 1 a 6 del proyecto, desde la configuración del entorno experimental hasta la evaluación comparativa de las cinco configuraciones definidas en la Tabla 4. Cada sección reporta tanto la decisión metodológica adoptada como la evidencia cuantitativa y cualitativa de su ejecución.
...
*Nota.* Elaboración propia a partir de los notebooks 03 a 07 de este trabajo. La cifra total de páginas OCR asciende a 1.094.
...
La cartilla narrativa *Antihua Pacay Gentecunapa Parlocuna* es la única fuente primaria disponible del dialecto Medio Putumayo y por tanto es estratégicamente relevante para la cobertura bidialectal del proyecto. El Notebook 06 segmenta el documento en sus dos mitades (un bloque continuo en Inga seguido por un bloque parcial en español) e intenta alinear las historias por orden de aparición. Sólo la primera de las seis historias del documento dispone de traducción al español, y la alineación oración a oración por índice presenta desfase significativo (91 oraciones en Inga frente a 102 en español). El resultado, 86 pares con calidad heterogénea, se documenta pero queda fuera del corpus de entrenamiento empleado en este estudio. La alineación correcta requiere un alineador semántico basado en embeddings multilingües, técnica que se identifica como vía de extensión natural del trabajo.
...
La adaptación de **NLLB-200-distilled-600M** al par Inga-español se ejecutó mediante adaptadores de bajo rango (LoRA) insertados sobre las proyecciones de consulta y valor (`q_proj`, `v_proj`) de las capas de atención del transformador. El entrenamiento corrió en una sola pasada sobre el corpus paralelo duplicado en ambas direcciones, de modo que un único adaptador sirve para inferir Inga-español o español-Inga según los códigos `src_lang` y `tgt_lang` que se pasen al tokenizador en inferencia. La duplicación del corpus de 4.512 a 9.024 ejemplos permitió compartir representaciones entre las dos direcciones sin necesidad de mantener dos adaptadores separados.

La configuración hiperparamétrica del entrenamiento se resume en la Tabla 7. La instrumentación utilizó las librerías *transformers*, *peft* y *accelerate* sobre Python 3.11 con backend Metal Performance Shaders. El procedimiento completo se documenta en el Notebook 09 del repositorio del proyecto, referenciado en el Anexo A.
...
*Nota.* Elaboración propia. El corpus de entrenamiento se duplica de 4.512 a 9.024 ejemplos para cubrir ambas direcciones de traducción.

La configuración de LoRA introdujo aproximadamente 1,18 millones de parámetros entrenables, equivalentes al 0,19 % de los 615 millones de parámetros totales del modelo base. Los pesos del adaptador resultante ocupan 9,4 megabytes, frente a los 2,4 gigabytes del modelo NLLB completo, lo que confirma la ventaja de eficiencia en almacenamiento y portabilidad que motiva esta familia de técnicas (Hu et al., 2022; Su et al., 2024). El tiempo total de entrenamiento sobre la estación local fue de aproximadamente 47 minutos para los 3 epochs y 3.384 *steps* de optimización.

La evolución de la función de pérdida durante el entrenamiento se muestra en la Figura 11. La pérdida de entrenamiento descendió de un valor inicial de 10,71 a 6,00 en los 3.384 *steps* totales, mientras que la pérdida de validación, evaluada al final de cada epoch, registró 6,05, 5,97 y 5,94 respectivamente. La convergencia entre las dos curvas, sin separación apreciable entre entrenamiento y validación, indica que el modelo no incurre en sobreajuste sobre el corpus disponible; sin embargo, la pérdida absoluta sigue siendo alta para los estándares de la traducción automática neuronal estándar, lo que sugiere que el adaptador alcanza el régimen lineal del proceso de aprendizaje pero no llega al punto de saturación que permitiría una calidad de traducción comparable a pares lingüísticos de alto recurso. Esta limitación se discute en relación con el tamaño efectivo del corpus en el Capítulo 5.
...
*Nota.* Elaboración propia a partir del Notebook 09. La curva azul muestra la pérdida de entrenamiento muestreada cada 20 *steps*. Los puntos rojos marcan la pérdida de validación al final de cada epoch.

La inspección cualitativa de las traducciones producidas por el adaptador revela un comportamiento asimétrico entre las dos direcciones. En la dirección Inga-español el modelo genera oraciones gramaticalmente correctas y semánticamente plausibles para versículos del Nuevo Testamento, aunque con desviaciones léxicas frecuentes respecto a la referencia. En la dirección español-Inga, en cambio, el decodificador entra en un modo de repetición patológica para más del 40 % de las oraciones del *val set*: secuencias del tipo "iapa iapa iapa..." o "suglla suglla suglla..." se prolongan hasta agotar el presupuesto de tokens, indicando que el modelo no ha logrado aprender una distribución de generación coherente para la lengua de bajos recursos a partir de los 4.512 pares disponibles. Este modo de fallo es conocido en la literatura sobre traducción a lenguas con datos extremadamente escasos y ha sido reportado tanto por Garcia Gilabert et al. (2024) como por DeGenaro y Lupicki (2024).

## 4.5 Pipeline LLM + RAG con Claude

**Contribución de Daniel Eslava.**

El segundo eje experimental opera sobre el modelo de lenguaje de frontera **Claude** de Anthropic, enriquecido con el contexto recuperado del sistema RAG descrito en la sección 4.3. La evaluación experimental se ejecutó con dos versiones del modelo (Sonnet 4.6 y Opus 4.7) para caracterizar el efecto del escalado del modelo subyacente sobre la calidad de traducción en condiciones de bajos recursos. El diseño del prompt es estructurado y específico por dirección. La dirección Inga-español utiliza un *system prompt* que sitúa al modelo como traductor especializado en una variante quechua del Putumayo y le pide responder únicamente con la traducción, sin explicaciones. La dirección español-Inga emplea un *system prompt* que enfatiza la primacía del vocabulario y los patrones gramaticales suministrados por el RAG sobre el conocimiento previo del modelo, dado el escaso volumen de Inga en los datos de preentrenamiento de cualquier modelo de lenguaje de frontera.

El cuerpo del prompt sigue un patrón consistente. La primera sección inyecta las cinco entradas léxicas más cercanas semánticamente a la oración a traducir, formateadas como `lema (categoría): glosa`. La segunda sección incluye hasta tres bloques de notas gramaticales del índice gramatical, truncados a 400 caracteres cada uno para no saturar el contexto. La tercera sección entrega cinco ejemplos paralelos similares en formato `INGA: ... / ESPAÑOL: ...`, invertido para la dirección opuesta. El prompt cierra con la instrucción explícita y la oración objetivo.

La inspección cualitativa de las traducciones producidas confirma que la inyección del contexto recuperado modifica sustancialmente el comportamiento del modelo. Un ejemplo ilustrativo proviene del versículo Lucas 9:17, donde el texto Inga "Tukuikunami sagsagta mikurkakuna. Nispaka, chunga iskai saparu junda piti pitikuna tandachirkakuna." debe traducirse como "Y comieron todos, y se hartaron; y alzaron lo que les sobró, doce cestos de pedazos". La traducción del adaptador LoRA produce "Y comieron todos los que quedaron, y se llenaron los pedazos, doce mil pedazos", incurriendo en una confusión numérica grave (doce mil en lugar de doce). El pipeline con Claude más RAG, por contraste, produce "Y comieron todos, y se hartaron; y alzaron lo que sobró de los pedazos, doce cestas llenas", una traducción casi literalmente equivalente al gold de referencia. Este patrón se repite en la mayoría de los ejemplos donde el RAG logra recuperar versículos paralelos cercanos del corpus de entrenamiento, situación que confirma el aporte del paradigma de aumentación por recuperación al rendimiento del modelo de frontera en este escenario de bajos recursos.

## 4.6 Evaluación comparativa

La evaluación comparativa se ejecutó sobre 100 ejemplos del *split* de validación seleccionados con semilla 42 para garantizar la reproducibilidad. La cantidad acotada responde a dos limitaciones operativas: el coste de las llamadas a la API de Anthropic asociadas al pipeline LLM con recuperación aumentada y el tiempo de evaluación local del modelo NLLB con generación por *beam search* de cuatro caminos. Los 564 ejemplos del *val set* completo y los 565 ejemplos del *test set* quedan disponibles para extensiones posteriores del trabajo, incluida la validación humana con hablantes nativos.

Las cinco configuraciones de la Tabla 4 del Capítulo 3 se evaluaron en las dos direcciones de traducción, totalizando diez corridas. Las métricas empleadas son BLEU corpus-level (sacrebleu), chrF++ con `word_order=2` (sacrebleu) y BERTScore F1 con el modelo XLM-RoBERTa multilingüe, configuradas de forma idéntica a las recomendaciones recientes de AmericasNLP (Ebrahimi et al., 2024; de Gibert et al., 2025). El pipeline LLM más RAG se ejecutó con Claude **Opus 4.7** como modelo principal de reporte. Los resultados consolidados se presentan en la Tabla 8.

**Tabla 8**
*Métricas BLEU, chrF++ y BERTScore por configuración y dirección sobre 100 ejemplos del val set*

| Config | Direccion | BLEU | chrF++ | BERTScore F1 |
|---|---|---|---|---|
| A: NLLB-200 zero-shot (quy_Latn) | Inga -> Espanol | 1,82 | 19,50 | 0,695 |
| A: NLLB-200 zero-shot (quy_Latn) | Espanol -> Inga | 0,16 | 15,88 | 0,674 |
| B: NLLB-200 + LoRA bidireccional | Inga -> Espanol | 8,18 | 27,19 | 0,752 |
| B: NLLB-200 + LoRA bidireccional | Espanol -> Inga | 1,79 | 22,89 | 0,676 |
| C: Claude Opus 4.7 zero-shot | Inga -> Espanol | 4,37 | 29,45 | 0,736 |
| C: Claude Opus 4.7 zero-shot | Espanol -> Inga | 0,32 | 21,75 | 0,722 |
| **D: Claude Opus 4.7 + RAG multi-indice** | **Inga -> Espanol** | **17,89** | **43,28** | **0,790** |
| **D: Claude Opus 4.7 + RAG multi-indice** | **Espanol -> Inga** | **10,95** | **38,19** | **0,788** |
...
Primero, la configuración D (Claude Opus 4.7 con recuperación aumentada multi-índice) obtiene el mejor desempeño en las dos direcciones de traducción y en las tres métricas reportadas. En la dirección Inga-español alcanza 17,89 puntos BLEU, 43,28 puntos chrF++ y 0,790 de BERTScore F1, cifras que sitúan al sistema en el lado alto del rango reportado por la literatura reciente para variantes quechuas con corpus comparables (Chen et al., 2024; Garcia Gilabert et al., 2024).

Segundo, la recuperación aumentada actúa como factor multiplicador de la calidad del modelo de frontera. El paso de la configuración C (Opus 4.7 sin RAG) a la configuración D mejora el BLEU de 4,37 a 17,89 en Inga-español, una ganancia de 13,5 puntos absolutos, y de 0,32 a 10,95 en español-Inga, una ganancia de 10,6 puntos. Este resultado confirma empíricamente la premisa del paradigma RAG-MT introducido por Wang et al. (2024) en el escenario específico de una lengua de recursos extremadamente bajos.

Tercero, el ajuste fino del modelo multilingüe (configuración B) ofrece una mejora sólida sobre el modelo zero-shot (configuración A) en la dirección Inga-español, donde el BLEU pasa de 1,82 a 8,18, un factor de 4,5x. La mejora en la dirección español-Inga es de mucha menor magnitud, de 0,16 a 1,79, debido al modo de fallo de repetición patológica descrito en la sección 4.4. La asimetría confirma que la generación de la lengua con datos escasos es estructuralmente más difícil que el reconocimiento de la lengua en el extremo fuente.

Cuarto, la configuración E (refinamiento de la salida del LoRA por Claude Opus 4.7 con RAG) no supera a la configuración D directa. En Inga-español, E obtiene 10,88 BLEU frente a los 17,89 de D, una pérdida de 7,0 puntos. En español-Inga las dos configuraciones quedan próximas (9,98 frente a 10,95). El resultado sugiere que la salida del adaptador LoRA, cuando se ofrece como semilla al modelo de frontera, induce un sesgo hacia traducciones menos fieles que la referencia, posiblemente porque el modelo de frontera tiende a respetar la estructura del *seed* en lugar de reescribir desde cero. La inspección manual de varios ejemplos muestra que Opus 4.7 hereda errores léxicos del LoRA y propaga ajustes solo en el plano sintáctico, lo que penaliza las métricas basadas en *n-grama*.

Quinto, BERTScore presenta menor variación entre configuraciones que BLEU y chrF++, lo cual es esperable dado que opera sobre representaciones contextuales que toleran reformulaciones semánticamente equivalentes. El rango de BERTScore F1 abarca de 0,674 (configuración A en español-Inga) a 0,790 (configuración D en Inga-español y configuración E en español-Inga). Esta compresión sugiere que las configuraciones evaluadas, incluso aquellas con BLEU muy bajo, producen salidas semánticamente parcialmente relacionadas con la referencia y que la diferencia perceptual entre ellas se concentra en la corrección léxica y morfológica capturada por las métricas basadas en superficie.
...
*Nota.* Elaboración propia a partir del Notebook 11. Cada panel corresponde a una métrica; las barras azules indican la dirección Inga-español y las rojas la dirección español-Inga. La configuración D destaca consistentemente en los tres paneles.
...
El trabajo demuestra la viabilidad técnica de construir un sistema bidireccional de traducción automática Inga-español a partir de fuentes primarias heterogéneas y mediante dos paradigmas complementarios. Cinco resultados concretos sustentan esta afirmación.
...
Cuarto, el pipeline LLM más RAG con Claude Opus 4.7 opera con *prompts* estructurados específicos por dirección, donde el *prompt* para español-Inga enfatiza la primacía del vocabulario y la gramática recuperados sobre el conocimiento previo del modelo, dado que Claude tiene exposición limitada al Inga durante su preentrenamiento. El sistema produce traducciones léxicamente fieles a la referencia en la mayoría de los versículos del *val set* y recupera correctamente la dirección español-Inga, evitando los modos de fallo de repetición observados en el adaptador NLLB.

Quinto, la evaluación comparativa de las cinco configuraciones experimentales (A, B, C, D y E) en ambas direcciones se ejecutó sobre 100 ejemplos del conjunto de validación, con métricas BLEU, chrF++ y BERTScore consolidadas en la Tabla 8. La configuración D (Claude Opus 4.7 con recuperación aumentada multi-índice) obtiene el mejor desempeño en las dos direcciones y en las tres métricas: 17,89 BLEU y 0,790 BERTScore F1 en Inga-español; 10,95 BLEU y 0,788 BERTScore F1 en español-Inga. La ganancia respecto al modelo de frontera sin recuperación (configuración C) es de 13,5 puntos BLEU en Inga-español y 10,6 puntos en español-Inga, cifras que confirman empíricamente el aporte del paradigma RAG-MT al rendimiento de un modelo de frontera operando sobre una lengua de recursos extremadamente bajos.

## 5.2 Discusión

Cuatro observaciones se derivan del cruce entre los hallazgos cuantitativos del Capítulo 4 y la literatura reciente sobre traducción de lenguas de bajos recursos.

La primera observación concierne al efecto del ajuste fino del modelo multilingüe respecto al modelo de frontera. La configuración B (NLLB-200 con LoRA bidireccional) mejora sustancialmente sobre el modelo base zero-shot (configuración A) en la dirección Inga-español, donde el BLEU asciende de 1,82 a 8,18. Sin embargo, la configuración D supera a B por un factor cercano a 2,2 en BLEU (17,89 frente a 8,18) y a 1,6 en chrF++ (43,28 frente a 27,19). Esta diferencia muestra que, en el régimen de datos paralelos extremadamente escasos del Inga, un modelo de frontera con acceso a contexto recuperado supera al ajuste fino de un modelo dedicado de menor tamaño. El resultado complementa lo observado por Enis y Hopkins (2024) sobre la *eficiencia de recursos* de Claude en pares de bajos recursos, y se alinea con la evidencia de Cahyawijaya, Lovenia y Fung (2024) sobre la efectividad del *in-context learning* con ejemplos cuidadosamente seleccionados.

La segunda observación es sobre la contribución específica de la recuperación aumentada al rendimiento del modelo de frontera. La configuración C (Claude Opus 4.7 zero-shot) obtiene únicamente 4,37 BLEU en Inga-español y 0,32 BLEU en español-Inga, cifras que confirman que el modelo de frontera, sin información adicional sobre la lengua, no resuelve el problema de traducción para el Inga. La incorporación del RAG multi-índice (configuración D) eleva estas cifras en órdenes de magnitud comparables a los reportados por Wang et al. (2024) y por Chen et al. (2024) para QueEn. La estructuración del conocimiento en tres índices independientes (léxico, gramatical y de ejemplos paralelos) constituye, en este sentido, una arquitectura efectiva para el problema, aunque la evidencia disponible no permite cuantificar la contribución marginal de cada índice por separado.
...
Cinco líneas de investigación se desprenden directamente de los hallazgos y las limitaciones identificadas.

Una primera línea consiste en incorporar el dialecto Medio Putumayo al corpus mediante un alineador semántico basado en *sentence embeddings* multilingües (LaBSE o similar) aplicado sobre las narrativas de *Antihua Pacay*. Una alineación por similitud coseno entre oraciones individuales puede recuperar entre 100 y 300 pares MP de calidad suficiente para incorporar el dialecto al *split* de entrenamiento y evaluar el efecto de la cobertura bidialectal en la calidad de la traducción.

Una segunda línea contempla un estudio sistemático de *ablations* sobre los hiperparámetros del adaptador LoRA, en particular sobre el rango (valores 4, 8, 16, 32, 64), el factor de escala alpha (16, 32, 64) y la incorporación de cuantización a 4 bits según la propuesta QLoRA de Dettmers et al. (2023). El estudio admite un complemento con augmentación sintética por *back-translation* sobre material monolingüe en Inga.

Una tercera línea contempla el escalado del modelo base de NLLB-200-distilled-600M a NLLB-200-3.3B, viable en la estación local de cómputo pero con tiempos de entrenamiento sustancialmente mayores. La comparación entre ambos tamaños permite evaluar el retorno marginal del aumento de parámetros para una lengua de recursos extremadamente bajos y contrastar el comportamiento con la evidencia reportada por Su et al. (2024) sobre PEFT en lenguas de bajos recursos.

Una cuarta línea consiste en la validación humana con hablantes nativos del Inga, idealmente con representación dialectal balanceada y rúbrica de tres dimensiones: adecuación semántica, fluidez gramatical y naturalidad cultural. La logística de la sesión se puede coordinar a través del integrante del equipo residente en Mocoa con compensación por participación según los principios éticos sistematizados por Mager et al. (2023).

Una quinta línea consiste en el empaquetado del sistema resultante como aplicación local reproducible, con interfaz de línea de comandos que acepta un texto en cualquiera de las dos lenguas y devuelve la traducción según la configuración óptima identificada (Claude Opus 4.7 con RAG multi-índice). La documentación incluye una guía de despliegue en hardware comparable al utilizado en este estudio y la indicación de que el adaptador LoRA, aunque no es el ganador absoluto, conserva valor como respaldo local cuando no hay conectividad con la API del modelo de frontera.
...
- `01_eda_recursos_base.ipynb`, análisis exploratorio de los cinco recursos lingüísticos primarios y primer volcado del *Diccionario Inga*.
...
La carpeta `entrega2/` contiene el documento maestro del trabajo y sus recursos gráficos asociados. El *script* `entrega2/build_docx_e2.py` genera bloques en formato Word a partir del Markdown maestro y la plantilla oficial UNIR, aplicando los estilos requeridos para captions APA y conservando los estilos de la plantilla.
