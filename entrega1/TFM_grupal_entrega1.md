...
La lengua Inga, variante de la familia quechua hablada por cerca de 18.000 personas en el departamento del Putumayo (Colombia), no cuenta con herramientas digitales de traducción automática. Este trabajo aborda esa brecha mediante el diseño y la implementación de un sistema comparativo Inga-Español construido sobre dos aproximaciones complementarias. La primera adapta un modelo de traducción multilingüe preentrenado al par Inga-Español mediante ajuste fino eficiente en parámetros, aprovechando las lenguas quechuas emparentadas que el modelo ya cubre como base de transferencia. La segunda construye un pipeline de traducción sobre un modelo de lenguaje de frontera potenciado con recuperación aumentada de información, sustentado en una base de conocimiento lingüística estructurada en tres índices independientes: léxico, gramatical y de ejemplos paralelos. El documento recoge el planteamiento del problema, la revisión del estado del arte con foco en la traducción automática del Quechua como antecedente más cercano, la definición de objetivos específicos bajo criterio SMART, la metodología organizada en siete fases y los avances iniciales del piloto experimental. Estos avances comprenden la configuración del entorno computacional sobre Apple Silicon, el inventario cuantificado de 1.094 páginas de material lingüístico primario provenientes de cinco fuentes (diccionario, gramática pedagógica, apéndice morfosintáctico, Nuevo Testamento en Inga y narrativas orales) y la extracción de un corpus inicial de 3.093 segmentos del Nuevo Testamento y 817 entradas léxicas estructuradas. La contribución original del trabajo reside en la cobertura bidialectal, Alto y Medio Putumayo, y en la comparación sistemática de ambas aproximaciones sobre el mismo par lingüístico.
...
The Inga language, a Quechua variant spoken by about 18,000 people in the department of Putumayo (Colombia), has no machine translation tools available. This work addresses that gap through the design and implementation of a comparative Inga-Spanish translation system built on two complementary approaches. The first adapts a pretrained multilingual translation model to the Inga-Spanish pair through parameter-efficient fine-tuning, taking advantage of the related Quechua languages that the model already covers as a transfer base. The second builds a translation pipeline on top of a frontier language model enhanced with retrieval-augmented generation, supported by a linguistic knowledge base structured in three independent indices: lexical, grammatical, and parallel-example. The document covers the problem statement, the state-of-the-art review focused on Quechua translation as the closest antecedent, the SMART-based definition of specific objectives, the seven-phase methodology, and the initial progress of the experimental pilot. This progress includes the configuration of the computational environment on Apple Silicon, a quantified inventory of 1,094 pages of primary linguistic material from five sources (dictionary, pedagogical grammar, morphosyntactic appendix, Inga New Testament, and oral narratives), and the extraction of an initial corpus of 3,093 New Testament segments and 817 structured lexical entries. The original contribution of the work lies in its bidialectal coverage, Alto and Medio Putumayo, and in the systematic comparison of both approaches on the same language pair.
